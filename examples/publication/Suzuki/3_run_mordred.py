...
for acq_i in [
    ...
    for seed_i in np.arange(0, 5):
        ...
        df_exp = pd.read_csv('./data/dataset_B3_Mordred_clean.csv')
        ...
        label_benchmark = f"benchmark_mordred_acq_{acq}_batch_{batch}_seed_{seed}.csv"

        if not os.path.exists(f"./results_mordred/{label_benchmark}"):
            ...
            # Move results.
            if not os.path.exists('results_mordred'):
                os.mkdir('results_mordred')
            shutil.move(label_benchmark, f'results_mordred/{label_benchmark}')
            shutil.move(f'pred_{label_benchmark}', f'results_mordred/pred_{label_benchmark}')
            shutil.move(f'results_{label_benchmark}', f'results_mordred/results_{label_benchmark}')
