...
for acq_i in [
    ...
    for seed_i in np.arange(0, 5):
        ...
        label_benchmark = f"benchmark_ohe_acq_{acq}_batch_{batch}_seed_{seed}.csv"

        if not os.path.exists(f"./results_ohe/{label_benchmark}"):
            ...
            # Move results.
            if not os.path.exists('results_ohe'):
                os.mkdir('results_ohe')
            shutil.move(label_benchmark, f'results_ohe/{label_benchmark}')
            shutil.move(f'pred_{label_benchmark}', f'results_ohe/pred_{label_benchmark}')
            shutil.move(f'results_{label_benchmark}', f'results_ohe/results_{label_benchmark}')
